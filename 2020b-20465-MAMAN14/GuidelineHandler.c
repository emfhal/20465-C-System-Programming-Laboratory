...
/* add data only if the counter smaller than DATA_SIZE value, incase is not return error*/
...
/* add data symbol by Guideline holder label name that he was not declared before*/
...
/*select the relevant handle 4 options by the start chars of the assembly line*/
...
void handleString(GuidelineHolder* holder, AssemblyLine* assemblyLine) {
	...
	assemblyLine->line = trim(assemblyLine->line);
		/* if the .string string value not between quotation marks return error */
...
void extractDataParams(GuidelineHolder* holder, AssemblyLine* assemblyLine)
{
	...
	while (token != NULL)
	{
		...
		/* if the .data values is not a number (+/-) return error */
		if ((*convertionEnd)) {
